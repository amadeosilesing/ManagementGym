...
import { SearchInput, FilterTabs, Pagination, Badge, Avatar } from '@/components/ui'
...
export default function PagosPage() {
  ...
  const [totales,    setTotales]    = useState({ general: 0, efectivo: 0, transferencia: 0, tarjeta: 0 })
  ...
  useEffect(() => {
    const timer = setTimeout(() => { setPage(1); fetchPagos(search, filtro, 1) }, 300)
    return () => clearTimeout(timer)
  }, [search])

  useEffect(() => { fetchPagos(search, filtro, page) }, [filtro, page])
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total registrado', valor: fmt(totales.general),       color: 'text-gray-900'   },
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="text-center py-12 text-gray-400 text-sm">
                  {search || filtro !== 'todos' ? 'No se encontraron resultados' : 'No hay pagos registrados'}
                </td>
              </tr>
            ) : (
              pagos.map((p) => {
