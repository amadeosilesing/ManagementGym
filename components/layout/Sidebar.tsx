...
export default function Sidebar() {
  ...
  const navFiltrado = navigation.filter((item) =>
    ...
    <aside className="w-60 min-h-screen bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="px-6 py-5 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">GymSystem</p>
            <p className="text-xs text-gray-400 capitalize">{session?.rol ?? '...'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-3 py-4 border-t border-gray-100 flex flex-col gap-1">
        {session && (
          <div className="px-3 py-2 mb-1">
            <p className="text-xs font-medium text-gray-900 truncate">{session.nombre}</p>
